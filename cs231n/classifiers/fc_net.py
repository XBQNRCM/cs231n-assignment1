...
import os
import numpy as np
from pandas.core.base import cache_readonly
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, affine_cache = affine_forward(affine_relu_out, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # 反向传播
        dout, grads['W2'], grads['b2'] = affine_backward(dscores, affine_cache)
        _, grads['W1'], grads['b1'] = affine_relu_backward(dout, affine_relu_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        pre_dim = input_dim
        for i, output_dim in zip(range(1, self.num_layers + 1), dims):
            self.params[f'W{i}'] = np.random.randn(pre_dim, output_dim) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # softmax layer
        softmax_w = self.params[f'W{self.num_layers}']
        softmax_b = self.params[f'b{self.num_layers}']
        scores = out @ softmax_w + softmax_b
        cache_list.append((out, softmax_w, softmax_b))
        ...
        for i in range(1, self.num_layers + 1):
            ...
            reg_loss += np.sum(self.params[f'b{i}'] ** 2)
        
        loss = data_loss + 0.5 * self.reg * reg_loss

        # softmax 层的梯度
        dx, dw, db = affine_backward(dscores, cache_list.pop())
